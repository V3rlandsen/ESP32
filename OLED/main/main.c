...
esp_err_t ssd1306_draw_bitmap_fast(uint8_t x, uint8_t y,const uint8_t *bitmap)
{
    memcpy(framebuffer, bitmap, 1024);
    return ESP_OK;
}
...
esp_err_t ssd1306_set_display_on(bool on){
    uint8_t cmd=on?SSD1306_DISPLAY_ON:SSD1306_DISPLAY_OFF;
    return ssd1306_write_command(cmd);
}

esp_err_t ssd1306_set_scroll_mode(bool rigth, uint8_t start_page, uint8_t end_page, uint8_t scroll_speed)
{
    ...
    uint8_t cmd[7];
    cmd[0] = rigth ? SSD1306_RIGHT_HORIZONTAL_SCROLL : SSD1306_LEFT_HORIZONTAL_SCROLL;
    ...
    return ssd1306_write_commands(cmd, 7);
}
esp_err_t ssd1306_set_scroll(bool scroll){
    return ssd1306_write_command(scroll?SSD1306_ACTIVATE_SCROLL:SSD1306_DEACTIVATE_SCROLL);
}
...
const uint8_t cat64x32[]={
    ...
    0x0f, 0x0f, 0x9f, 0x9f, 0x1f, 0x1f, 0x3f, 0x1f, 0x0f, 0x8f, 0x0f, 0x8f, 0x0f, 0x0f, 0x07, 0x07, 
    0xe7, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 
    0xff, 0xff, 0xff, 0xff, 0x7f, 0x7f, 0x3f, 0x3f, 0x3f, 0x3f, 0x1f, 0x1f, 0x07, 0x00, 0x00, 0x00, 
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
    ...
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x9f, 0x8f, 0x8e, 0x80, 0x80, 0x80, 0x80, 0x80, 
    0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0xc0, 0xc0, 0xc0, 
    0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0xc0, 0xd8, 0xc0, 0xc0, 0x80, 0x80, 0x80, 0x9c, 0x9f, 
...
void app_main(void)
{
    ...
    {   
        ...
        ssd1306_draw_bitmap_fast(0, 0, cat128x64);
        ESP_ERROR_CHECK(ssd1306_update());
        vTaskDelay(pdMS_TO_TICKS(500));
        //ssd1306_set_scroll(false);
        vTaskDelay(pdMS_TO_TICKS(500));
    }
}
